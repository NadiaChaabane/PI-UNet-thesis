...
from layout_data.utils.custom_transform import  GaussianBlur, AddGaussianNoise
...
class UnetUL(LightningModule):
    ...
    def prepare_data(self):
        ...
        transform_layout_noise = transforms.Compose([
            #GaussianBlur(kernel_size=self.hparams.blur_kernel_size,
                       #sigma=self.hparams.blur_sigma),
            transforms.ToTensor(),
            transforms.Normalize(
                torch.tensor([self.hparams.mean_layout]),
                torch.tensor([self.hparams.std_layout]),
            ),
            AddGaussianNoise(std=getattr(self.hparams,'noise_std',0.0)),
        ])

        # Validation and test uses clean transform
        transform_layout_clean = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize(
                torch.tensor([self.hparams.mean_layout]),
                torch.tensor([self.hparams.std_layout]),
        ...
        train_dataset = LayoutDataset(
            ...
            transform=transform_layout_noise, target_transform=transform_heat,
        ...
        val_dataset = LayoutDataset(
            ...
            transform=transform_layout_clean, target_transform=transform_heat,
            load_name=self.hparams.load_name, nx=self.hparams.nx,
        )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Only visualize for first batch and selected epochs
        if batch_idx == 0 and self.current_epoch in [0, 5, 10, 15, 20, 29]:
            N, _, _, _ = heat.shape
            heat_list, heat_pre_list = [], []
            for heat_idx in range(min(5, N)):
            ...
            visualize_heatmap(
                x, y, heat_list, heat_pre_list,
                self.current_epoch,
                getattr(self.hparams, "blur_kernel_size", 0),
                getattr(self.hparams, "blur_sigma", 0.0),
                getattr(self.hparams, "noise_std", 0.0),
            )

        return {"val_loss_jacobi": loss_jacobi, "val_mae": val_mae}
    ...
    #         x = np.linspace(0, 0.1, self.hparams.nx)
    #         y = np.linspace(0, 0.1, self.hparams.nx)
    #         visualize_heatmap(x, y, heat_list, heat_pre_list, self.current_epoch, self.hparams.blur_kernel_size, self.hparams.blur_sigma)
