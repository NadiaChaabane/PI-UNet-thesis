...
def visualize_heatmap(x, y, heat_list, heat_pre_list, epoch, ks, sg, noise):
    ...
    for i in range(num):
        ...
        plt.colorbar()
        plt.title('Error')
    plt.savefig(f'figures_simple_noise/epoch{epoch}_noise{noise}_pre.png', bbox_inches='tight', pad_inches=0)
    plt.close()
