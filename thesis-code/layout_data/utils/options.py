...
def parses_ul(config_path):
    ...
    parser.add_argument(
        "--use_16bit", type=lambda x: str(x).lower() in ['true', '1', 'yes'], 
        default=False, help="use 16bit precision"
    )
    ...
    # noise parameter
    parser.add_argument("--noise_std", default=0.0, type=float, help="Standard deviation for Additive Gaussian noise")
    parser.add_argument('--blur_sigma', type=float, default=0.0,
                    help='Gaussian blur sigma (in pixels) on normalized φ at test time')
    parser.add_argument('--blur_kernel_size', type=int, default=5,
                    help='Odd kernel size for blur; will be bumped to odd if even')
    # dataset params
